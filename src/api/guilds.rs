...
use crate::authentication::scopes::{ARISTOCRAT_GUILDS_MODIFY};
use crate::authentication::Claims;
use crate::models::{Id, NewGuild};
use actix_web::{web, HttpResponse};

pub async fn get_all(app: web::Data<AppState>) -> MyResponder {
...
pub mod collection {
    ...
    pub async fn get_all(
        ...
        if query.recursive {
            let books = actions::book::double_recursive_list_owned_by(&conn, guild.into())?;
    ...
    pub mod titles {
        ...
        use crate::api::MyResponder;
        use crate::app::AppState;
        use crate::models::{Id, Owner, QueryOptions};
        ...
        pub async fn get_all(
            ...
            let guild = Owner::from(actions::guild::find(&conn, *guild_id)?);
            if query.recursive {
                let titles = actions::title::recursive_list_owned_by(&conn, guild.into())?;
                Ok(HttpResponse::Ok().json(titles))
            } else {
                let titles= actions::title::list_owned_by(&conn, guild.into())?;
                Ok(HttpResponse::Ok().json(titles))
            }
        }
    ...
    pub mod rpgsystems{
        ...
        pub async fn get_all(
            ...
            // doesn't exist and we are more consistent with the other endpoints.
            let guild = Owner::from(actions::guild::find(&conn, *guild_id)?);
                let systems = actions::rpg_system::list_owned_by(&conn, guild.into())?;
